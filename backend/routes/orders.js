...
router.post('/', [
  ...
  body('paymentMethod').optional().isIn(['cash', 'card', 'upi', 'wallet']).withMessage('Invalid payment method')
], async (req, res) => {
  try {
    ...
    const { items, pickupAddress, pickupDate, pickupTime, deliveryAddress, specialInstructions, paymentMethod } = req.body;
    ...
    const order = new Order({
      ...
      paymentStatus: (paymentMethod && paymentMethod !== 'cash') ? 'paid' : 'pending'
    });
